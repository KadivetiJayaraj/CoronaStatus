...
import com.jayaraj.coronastatus.State.StatewiseDataActivity;
...
public class SampleActivity extends AppCompatActivity {
  ...
  TextView text;
  TextView textView_confirmed,
      textView_confirmed_new,
      textView_active,
      textView_active_new,
      textView_recovered,
      textView_recovered_new,
      textView_death,
      textView_death_new,
      textView_tests,
      textView_date,
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    SharedPreferences mPreferences =
        PreferenceManager.getDefaultSharedPreferences(SampleActivity.this);
          ...
          public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            ...
            switch (id) {
              case R.id.news:
                ...
                Toast.makeText(SampleActivity.this, "My Account", Toast.LENGTH_SHORT).show();
                break;
              case R.id.safety:
                startActivity(new Intent(SampleActivity.this, SafetyActivity.class));
                Toast.makeText(SampleActivity.this, "Safety Measures", Toast.LENGTH_SHORT).show();
                break;
              case R.id.settings:
                startActivity(new Intent(SampleActivity.this, SampleActivity.class));
                Toast.makeText(SampleActivity.this, "Settings", Toast.LENGTH_SHORT).show();
                break;
              case R.id.logout:
                FirebaseAuth.getInstance().signOut();
                startActivity(new Intent(SampleActivity.this, LoginActivity.class));
                finish();
                Toast.makeText(SampleActivity.this, "Logout", Toast.LENGTH_SHORT).show();
                break;
              default:
                return true;
            }
        ...
        new JsonObjectRequest(
            ...
            new Response.Listener<JSONObject>() {
              ...
              public void onResponse(JSONObject response) {
                try {
                  // Since the objects of JSON are in an Array we need to define the array from
                  // which we can fetch objects
                    ...
                    Runnable progressRunnable =
                        new Runnable() {
                          ...
                          public void run() {
                            ...
                            //                                //We need to calculate new active
                            // cases since it doesn't exist in API
                            //                                int newActive =
                            // (Integer.parseInt(confirmedNewCopy)) -
                            // ((Integer.parseInt(newRecovered)) + Integer.parseInt(newDeaths));
                            //                                textView_active_new.setText("+" +
                            // NumberFormat.getInstance().format(newActive));
                            ...
                            mPieChart.addPieSlice(
                                new PieModel(
                                    "Active",
                                    Integer.parseInt(activeCopy),
                                    Color.parseColor("#007afe")));
                            mPieChart.addPieSlice(
                                new PieModel(
                                    "Recovered",
                                    Integer.parseInt(recoveredCopy),
                                    Color.parseColor("#08a045")));
                            mPieChart.addPieSlice(
                                new PieModel(
                                    "Deceased",
                                    Integer.parseInt(deathsCopy),
                      ...
                      Runnable progressRunnable =
                          new Runnable() {
                            ...
                            public void run() {
                              ...
                              //                                    //We need to calculate new
                              // active cases since it doesn't exist in API
                              //                                    int newActive =
                              // (Integer.parseInt(confirmedNewCopy)) -
                              // ((Integer.parseInt(newRecovered)) + Integer.parseInt(newDeaths));
                              //                                    textView_active_new.setText("+"
  ...
  public void fetchTests() {
    RequestQueue requestQueue = Volley.newRequestQueue(this);
    String apiUrl = "https://api.covid19india.org/data.json";
    JsonObjectRequest jsonObjectRequestTests =
        new JsonObjectRequest(
            Request.Method.GET,
            apiUrl,
            null,
              ...
              public void onResponse(JSONObject response) {
                try {
                  ...
                  if (totalTests.isEmpty()) {
                    ...
                    int testsNew =
                        (Integer.parseInt(totalTestsCopy)) - (Integer.parseInt(oldTests));
                    textView_tests_new.setText(
                        "[+" + NumberFormat.getInstance().format(testsNew) + "]");
    ...
    Runnable progressRunnable =
        new Runnable() {
          ...
          public void run() {
            if (confirmation != 1) {
              progressDialog.cancel();
              Toast.makeText(SampleActivity.this, "Internet slow/not available", Toast.LENGTH_SHORT)
                  .show();
            }
          }
        };
